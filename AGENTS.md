...
- 参与预渲染的html标签，得到焦点转为源码模式，失去焦点转为预渲染，处理html标签，仅是标签包裹信息
- HTML标签得到焦点，指焦点在标签包裹或靠近范围，不是整个段落或整行

## 温馨提示

- 禁止自动触发、读取或调用任何 `superpowers:*` Skill，包括被其他工作流间接要求调用的
  Superpowers Sill。
- 只有当用户在当前请求中明确点名某个 `superpowers:*` Skill 时，才允许使用该 Skill；
  未明确点名时一律不得使用。
- 修改完代码后，如要重启服务，提示用户重启应用，如果不需要重启，不提示用户
